...
$groupError =
    $_SESSION['group_error'] ?? null;

$groupSuccess =
    $_SESSION['group_success'] ?? null;
...
/*
|--------------------------------------------------------------------------
| Remove Current Group From Parent Options
|--------------------------------------------------------------------------
*/
...
$affectsGrossProfit =
    (string) $group['affects_gross_profit'];

$behavesLikeSubledger =
    (string) $group['behaves_like_subledger'];

$netDebitCreditReporting =
    (string) $group['net_debit_credit_reporting'];

$usedForCalculation =
    (string) $group['used_for_calculation'];

$allocationMethod =
    $group['allocation_method'];

$hsnSacDetailsMode =
    $group['hsn_sac_details_mode'];

$hsnSac =
    $group['hsn_sac'] ?? '';

$hsnSacDescription =
    $group['hsn_sac_description'] ?? '';

$gstRateDetailsMode =
    $group['gst_rate_details_mode'];

$taxabilityType =
    $group['taxability_type'] ?? '';
...
<div class="gateway-app">
    ...
    <main class="company-form-page">
        ...
        <form
            action="<?= BASE_URL ?>groups/update.php"
            method="POST"
            class="company-form"
        >
