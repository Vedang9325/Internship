...
// Sort Groups: System groups first, followed by user-created groups.
usort($groups, static function (array $a, array $b): int {
    if ((int)$a['is_system'] !== (int)$b['is_system']) {
        return (int)$b['is_system'] <=> (int)$a['is_system'];
    }
...
<div class="gateway-app">
    ...
    <main class="company-select-page">
        <section class="company-select-panel">
            <div class="company-select-title">Select Group to Alter</div>
            ...
            <div class="company-select-list">
                <?php if (empty($groups)): ?>
                    <div class="company-select-empty">No groups available.</div>
                <?php else: ?>
                    <?php foreach ($groups as $group): ?>
                        <a href="<?= BASE_URL ?>groups/edit.php?id=<?= (int)$group['id'] ?>" class="company-select-item">
                            <span><?= htmlspecialchars($group['name']) ?></span>
                            <?php if ((int)$group['is_system'] === 1): ?>
                                <span class="company-active-label">SYSTEM</span>
                            <?php else: ?>
                                <span>USER</span>
                            <?php endif; ?>
                        </a>
