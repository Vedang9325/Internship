...
$switchError = $_SESSION['company_switch_error'] ?? null;
...
<head>
    ...
    <title>Select Company | FinnServ</title>
...
<div class="gateway-app">
    ...
    <div class="gateway-titlebar">
        ...
        <div class="gateway-title-company"><?= htmlspecialchars($_SESSION['company_name'] ?? 'No Company') ?></div>
    </div>

    <main class="company-select-page">
        <section class="company-select-panel">
            <?php if ($switchError): ?>
                <div class="form-error"><?= htmlspecialchars($switchError) ?></div>
            <?php endif; ?>
            <div class="company-select-title">Select Company</div>
            <div class="company-select-list">
                <?php if (empty($companies)): ?>
                    <div class="company-select-empty">No companies available.</div>
                <?php endif; ?>
                <?php foreach ($companies as $company): ?>
                    <a href="<?= BASE_URL ?>company/switch.php?id=<?= (int)$company['id'] ?>" class="company-select-item <?= (int)$company['id'] === $activeCompanyId ? 'active' : '' ?>">
                        <span><?= htmlspecialchars($company['name']) ?></span>
                        <?php if ((int)$company['id'] === $activeCompanyId): ?>
            ...
            <div class="company-select-actions">
                <a href="<?= BASE_URL ?>company/create.php" class="company-action-button">Create Company</a>
